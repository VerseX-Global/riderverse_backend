...
exports.logoutVendor = exports.loginVendor = exports.registerVendor = void 0;
...
async function registerVendor(request, res) {
    ...
    try {
        const validationRules = [
            ...
            (0, express_validator_1.body)('operating_area').isArray().withMessage('Operating Area is required and must be an array'),
    ...
    Powered by RiderVerse.net`;
        ...
        const token = jsonwebtoken_1.default.sign({ vendorId: newvendor.id, email: newvendor.email, fullname: newvendor.fullname, phone_number: newvendor.phone_number, username: newvendor.username, profile_image: newvendor.profile_image, role: 'Vendor' }, config_1.Config.secret);
...
async function loginVendor(req, res) {
    ...
    try {
        ...
        // Verify the password
        const passwordMatch = await bcrypt_1.default.compare(password, vendor.password);
        if (!passwordMatch) {
            res.status(401).json({ error: 'Invalid email or password' });
            return;
        }
        // Generate a JWT token for the vendor
        const token = jsonwebtoken_1.default.sign({ vendorId: vendor.id, email: vendor.email }, config_1.Config.secret);
